...
export ICON_REPO_CONFIG_FILE=deployments/dev/app/configs/dev-oidc.json
export deployment_target=k8s           # "local" or "k8s"
export backend_client_split=cdn-origin # "cors" or "cdn-origin"

set +x
ui_bundle=web/dist/bundle.js
ui_bundle_dir=web/dist

settle_down_secs=1

LOGS_HOME=~/workspace/logs
mkdir -p $LOGS_HOME
webpack_log=$LOGS_HOME/iconrepo-webpack-build
...
watch_webpack() {
  tail -F -n5000 $webpack_log | while IFS= read -r line; do
    if echo "$line" | grep 'webpack.*compiled successfully'; then
