...
var testResponseIconData []web.ResponseIcon

var moreTestResponseIconData []web.ResponseIcon
...
func init() {
	...
	testResponseIconData = []web.ResponseIcon{}
	for _, testIconDescriptor := range testIconInputDataDescriptor {
		testResponseIconData = append(testResponseIconData, web.CreateResponseIcon("/icon", mapIconfileSizes(testIconDescriptor)))
	}

	moreTestResponseIconData = []web.ResponseIcon{}
	for _, testIconDescriptor := range moreTestIconInputDataDescriptor {
		moreTestResponseIconData = append(moreTestResponseIconData, web.CreateResponseIcon("/icon", mapIconfileSizes(testIconDescriptor)))
	}
}
...
func getTestData(icons []domain.Icon, responseIcons []web.ResponseIcon) ([]domain.Icon, []web.ResponseIcon) {
	...
	responseIconListClone := []web.ResponseIcon{}
	for _, resp := range responseIcons {
		paths := make([]web.IconPath, len(resp.Paths))
		tags := make([]string, len(resp.Tags))
		copy(paths, resp.Paths)
		copy(tags, resp.Tags)
		respClone := web.ResponseIcon{
			Name:       resp.Name,
			Paths:      paths,
			Tags:       tags,
...
func Get() ([]domain.Icon, []web.ResponseIcon) {
	return getTestData(testIconInputDataMaster, testResponseIconData)
}

func GetMore() ([]domain.Icon, []web.ResponseIcon) {
